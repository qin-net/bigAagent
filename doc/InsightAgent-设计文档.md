...
请金融同学交付两套，而不是仅 60 条大师原话：

1. **可执行规则**：输入字段、阈值、通过/失败/不适用  
2. **对应条文**：规则命中后给模型写解释用的短文本（可带来源）

检索应数据触发：例如「利润增、经营现金流不增」才召回盈利质量相关条文，禁止每票固定关键词召回同一批语录。

Agent 侧：跑规则 → 强制 citations 指向 `rule_id` 或字段 → 模型只解释命中/未命中。
...
方法论条目告诉 Agent「应该检查什么、适用边界是什么」，不直接作为买卖信号。

### 9.2 Case 与晋升闸门

跟踪与复盘产出 Case，不直接改 Prompt 或买卖权重。Case 至少包含：当时 thesis、falsifiers、后来发生的事实、过程错误（漏证据、不该判却判了）、是否仍成立（基于新事实）。
...
评测看过程：该不该弃权、风险有没有点到、句证是否一致、价值与时机是否分开。不看随后几天涨跌。
